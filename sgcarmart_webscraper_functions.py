...
def postdate_retrieval(parsed_listing_url):
    
    post_date = parsed_listing_url.find("div", {"id": "usedcar_postdate"}).text.split()[2]

    return post_date

def all_retrieval(parsed_listing_url):
    title_dict={}
    all_cats = parsed_listing_url.find_all(class_='eachInfo')
    for i in range(len(all_cats)):
        row_title=all_cats[i].find(class_='row_title')
        title=row_title.findChildren()
        
        row_info=all_cats[i].find(class_='row_info').text.strip()
        if len(title)!=0:
            title_dict[title[0].text]=row_info
...
def road_tax_retrieval(parsed_listing_url):
    string_data = all_retrieval(parsed_listing_url)['Road Tax']
...
def mileage_retrieval(parsed_listing_url):
    ...
    data_value =all_retrieval(parsed_listing_url)['Mileage']
...
def manufactured_year_retrieval(parsed_listing_url):
    manufactured_year = all_retrieval(parsed_listing_url)['Manufactured'].strip()
    return manufactured_year

# Transmission Retriever
def transmission_retrieval(parsed_listing_url):
    transmission = all_retrieval(parsed_listing_url)['Transmission']
    return transmission
...
def dereg_value_retrieval(parsed_listing_url):
    ...
    data_value = all_retrieval(parsed_listing_url)['Dereg Value'].strip().split() 
...
def omv_retrieval(parsed_listing_url):    
    data_value = all_retrieval(parsed_listing_url)['OMV'].split('$') 
...
def arf_retrieval(parsed_listing_url):
    data_value = all_retrieval(parsed_listing_url)['ARF'].split('$')
...
def coe_retrieval(parsed_listing_url):
    data_value = all_retrieval(parsed_listing_url)['COE'].split('$')
...
def engine_capacity_retrieval(parsed_listing_url):
    data_value = all_retrieval(parsed_listing_url)['Engine Cap'].strip().split('cc')
...
def curb_weight_retrieval(parsed_listing_url):
    data_value = all_retrieval(parsed_listing_url)['Curb Weight'].split()
...
def number_of_owners_retrieval(parsed_listing_url):
    no_of_owners = int(all_retrieval(parsed_listing_url)['No. of Owners'])
    return no_of_owners
